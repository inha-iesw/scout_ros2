...
void ScoutBaseRos::LoadParameters()
{
  ...
  RCLCPP_INFO_STREAM(this->get_logger(), "- odom topic name: " << odom_topic_name_);

  RCLCPP_INFO_STREAM(this->get_logger(), "- is scout mini: " << std::boolalpha << is_scout_mini_);
  RCLCPP_INFO_STREAM(this->get_logger(), "- is omni wheel: " << std::boolalpha << is_omni_wheel_);
  RCLCPP_INFO_STREAM(this->get_logger(), "- auto reconnect: " << std::boolalpha << auto_reconnect_);

  RCLCPP_INFO_STREAM(this->get_logger(),
                     "- simulated robot: " << std::boolalpha << simulated_robot_);
  RCLCPP_INFO_STREAM(this->get_logger(), "- sim control rate: " << sim_control_rate_);
  RCLCPP_INFO_STREAM(this->get_logger(), "----------------------------");
}
  ...
  if (detector.Connect(port_name_))
  {
    ...
    if (proto == ProtocolVersion::AGX_V1)
    {
      ...
      else
      {
        is_omni_ = true;
        omni_robot_ =
          std::unique_ptr<ScoutMiniOmniRobot>(new ScoutMiniOmniRobot(ProtocolVersion::AGX_V1));
        RCLCPP_INFO_STREAM(this->get_logger(),
    ...
    else if (proto == ProtocolVersion::AGX_V2)
    {
      ...
      if (!is_omni_wheel_)
      {
        ...
        RCLCPP_INFO_STREAM(this->get_logger(), "Creating interface for Scout with AGX_V2 Protocol");
      }
      else
      {
        is_omni_ = true;
        omni_robot_ =
          std::unique_ptr<ScoutMiniOmniRobot>(new ScoutMiniOmniRobot(ProtocolVersion::AGX_V2));
        RCLCPP_INFO_STREAM(this->get_logger(),
...
void ScoutBaseRos::Run()
{
  ...
  if (!is_omni_)
  {
    std::unique_ptr<ScoutMessenger<ScoutRobot>> messenger =
      std::unique_ptr<ScoutMessenger<ScoutRobot>>(new ScoutMessenger<ScoutRobot>(robot_, this));
    ...
    if (port_name_.find("can") != std::string::npos)
    {
      if (robot_->Connect(port_name_))
      {
        ...
        RCLCPP_INFO_STREAM(this->get_logger(), "Using CAN bus to talk with the robot");
      }
      else
      {
        RCLCPP_INFO_STREAM(this->get_logger(), "Failed to connect to the robot CAN bus");
        return;
      }
    }
    else
    {
      RCLCPP_INFO_STREAM(this->get_logger(), "Please check the specified port name is a CAN port");
      return;
    }
